...
void setup()
{
  ...
  //  if (Firebase.set(firebaseData2, path + "log/system_start", timestampNow))
  //  {
...
void printResult(StreamData & data)
{
  ...
  else if (data.dataType() == "json")
  {
    ...
    JsonObject& recieve = jsonBuffer.parseObject(jsonStr);
    if (data.dataPath() == "/timer/timer1") {
      bool gett1Status = recieve["status"];
      int gett1Hour = recieve["hour"];
      int gett1Minute = recieve["minute"];
      int gett1Duration = recieve["duration"];
      if (recieve.containsKey("status")) {
        t1Status = gett1Status;
      } else if (recieve.containsKey("hour")) {
        t1Hour = gett1Hour;
      } else if (recieve.containsKey("minute")) {
        t1Minute = gett1Minute;
      } else if (recieve.containsKey("duration")) {
        t1Duration = gett1Duration;
      }
      ...
      bool getrgb = recieve["status"];
      String getrgbMode = recieve["mode"];
      if (recieve.containsKey("status")) {
        rgb = getrgb;
      } else if (recieve.containsKey("mode")) {
        rgbMode = getrgbMode;
      }
      //      if (key == "status") {
      //        rgb = value == "true" ? true : false;
      ...
      String getcmd = recieve["value"];
      if (recieve.containsKey("value")) {
        cmd = getcmd;
      }
      //      if (key == "value") {
      //        cmd = value;
